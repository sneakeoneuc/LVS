...
param (
    ...
    [Int]$BuildDefinitionID = 64,
    [Boolean]$IncludeLatestBuilds = $False
)
...
$APIURL = "https://dev.azure.com/{{Org}}/{{Project}}/_apis/build/definitions/{{BuildDefinitionID}}?api-version=6.0&includeLatestBuilds={{IncludeLatestBuilds}}"
...
if ($IncludeLatestBuilds){
    $APIURL = $APIURL -replace "{{IncludeLatestBuilds}}", "true"
}else{
    $APIURL = $APIURL -replace "{{IncludeLatestBuilds}}", "false"
}
