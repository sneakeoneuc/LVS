...
# Restrict core dumps
echo "#Test=none" >> /etc/systemd/coredump.conf
...
## note: file path was /etc/ssh/sshd_config on my local system, /etc/ssh/sshd_conf in the doc

# Commenting SSH config previous entries
sed -re 's/^(X11Forwarding)([[:space:]]+)(.*)/#\1\2\3/' -i.`date -I` /etc/ssh/sshd_config
sed -re 's/^(LogLevel)([[:space:]]+)(.*)/#\1\2\3/' -i.`date -I` /etc/ssh/sshd_config
sed -re 's/^(HostbasedAuthentication)([[:space:]]+)(.*)/#\1\2\3/' -i.`date -I` /etc/ssh/sshd_config
sed -re 's/^(PermitUserEnvironment)([[:space:]]+)(.*)/#\1\2\3/' -i.`date -I` /etc/ssh/sshd_config
sed -re 's/^(AllowTcpForwarding)([[:space:]]+)(.*)/#\1\2\3/' -i.`date -I` /etc/ssh/sshd_config

# Inserting new SSH_config entries
echo "X11Forwarding no" >> /etc/ssh/sshd_conf
echo "LogLevel INFO" >> /etc/ssh/sshd_conf
echo "HostbasedAuthentication no" >> /etc/ssh/sshd_conf
echo "PermitUserEnvironment no" >> /etc/ssh/sshd_conf
echo "AllowTcpForwarding no" >> /etc/ssh/sshd_conf
...
# check crontab for aide entry. If not, add the following
# 0 5 * * * /usr/sbin/aide --check
# not sure how to check this one, tbd

# Configure the operating system to prohibit password reuse.
sed -i -e 's/^password\s*sufficient\s.*/password    sufficient    pam_unix.so sha512 shadow nullok try_first_pass use_authtok remember=5/g' /etc/pam.d/system-auth

# Configure Mail Transfer Agents (MTA)
sed -i -e 's/^inet_interfaces = .*/inet_interfaces = localhost/g' /etc/postfix/main.cf
